...
def _cost_band_section(kpi: pd.DataFrame) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "出稿を増やすと「登録→購入」が落ちてCPAが上がる。週次13週を日次Cost 40万円で"
            "二分すると、低出稿群（平均27.0万/日）は登録→購入62.3%・CPA 7,172円・"
            "粗利190,781円/日、高出稿群（平均56.8万/日）は50.5%・11,187円・78,908円/日で、"
            "**粗利/日が2.4倍違う**。",
            "",
            "ただし帯ごとの「赤字日」列だけを見て単日の合否を判断しないこと"
            "（月をまたぐと客単が違うため、Cost高＝赤字にはならない）。単日の合否は"
            "上のCPA帯で見る。",
        ]
    )
